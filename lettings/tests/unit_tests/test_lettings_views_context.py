...
def test_lettings_index_context_contains_lettings_list(client):
    ...
    addr2 = Address.objects.create(number=2,
                                  ...
                                  zip_code=21000,
                                  country_iso_code="FRA")
    letting_1 = Letting.objects.create(title="Letting 1", address=addr1)
    letting_2 = Letting.objects.create(title="Letting 2", address=addr2)

    response = client.get(reverse("lettings_index"))

    lettings_list = response.context["lettings_list"]
    titles = {l.title for l in lettings_list}
